...
const LIKELIHOOD_COLORS: Record<string, { bg: string; text: string }> = {
  "Most Likely": { bg: "rgba(59,130,246,0.14)", text: "var(--accent)" },
...
export default async function SharedDiagnosisPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div style={{ minHeight: "100dvh", backgroundColor: "#0d0f12", fontFamily: "var(--font-inter, system-ui, sans-serif)" }}>
      {/* Header */}
      <div style={{ position: "sticky", top: 0, zIndex: 10, height: "52px", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: "#0d0f12", borderBottom: "1px solid var(--border)" }}>
        <Link href="/" style={{ textDecoration: "none" }}>
          <span style={{ fontWeight: 900, fontSize: "18px", color: "var(--accent)", letterSpacing: "0.15em" }}>CARLOS</span>
        </Link>
        {isOwner && (
          <div style={{ display: "flex", alignItems: "center", gap: "6px", backgroundColor: "#1a1e25", border: "1px solid #252b34", borderRadius: "20px", padding: "4px 12px" }}>
            ...
            <span style={{ fontSize: "12px", color: "var(--accent)", fontWeight: 600 }}>{view_count} views</span>
          </div>
        )}
        {!isSignedIn && (
          <Link href="/" style={{ fontSize: "12px", fontWeight: 600, padding: "5px 12px", borderRadius: "20px", border: "1px solid rgba(59,130,246,0.5)", color: "var(--accent)", backgroundColor: "rgba(59,130,246,0.08)", textDecoration: "none" }}>
            Try Carlos
          </Link>
        )}
        {isSignedIn && !isOwner && (
      ...
      <div style={{ padding: "16px 16px", maxWidth: "560px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        {topCause && (
          <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderLeft: "3px solid var(--accent)", borderRadius: "10px", padding: "14px 16px" }}>
            ...
            <span style={{
              ...
              color: LIKELIHOOD_COLORS[topCause.likelihood]?.text ?? "var(--accent)",
        ...
        {topEst && (
          <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "13px", color: "#6b7280" }}>Est. Repair Cost</span>
            <span style={{ fontSize: "22px", fontWeight: 800, color: "var(--accent)" }}>{topEst.total}</span>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          {[
            ...
            <div key={stat} style={{ flex: 1, backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "8px", padding: "8px", textAlign: "center", fontSize: "11px", color: "#6b7280", fontWeight: 500 }}>
              {stat}
            </div>
          ))}
        </div>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ filter: isSignedIn ? "none" : "blur(5px)", userSelect: isSignedIn ? "auto" : "none", pointerEvents: isSignedIn ? "auto" : "none" }}>
              <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px 16px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {diagnosis.rankedCauses.slice(1).map((cause: { rank: number; cause: string; likelihood: string }) => {
                    ...
                    return (
                      <div key={cause.rank} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", backgroundColor: "#1a1e25", borderRadius: "8px", border: "1px solid var(--border)" }}>
              ...
              {diagnosis?.diagnosticSteps && (
                <div style={{ marginTop: "12px", backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px 16px" }}>
                  ...
                  {diagnosis.diagnosticSteps.slice(0, 2).map((step: { step: number; action: string }) => (
                    <div key={step.step} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
        ...
        {isSignedIn && diagnosis?.costEstimates && (
          <div style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px 16px" }}>
            ...
            {diagnosis.costEstimates.map((est: { fix: string; total: string; parts: string; labor: string; note?: string }, i: number) => (
              <div key={i} style={{ backgroundColor: "#1a1e25", border: "1px solid var(--border)", borderRadius: "8px", padding: "12px", marginBottom: i < diagnosis.costEstimates.length - 1 ? "8px" : 0 }}>
